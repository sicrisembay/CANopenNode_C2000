        ...
        uint16_t                CANbitRate)
{
    ...
    if(ECanRegPtr == &ECanaRegs) {
        hwiHdl_can = Hwi_create(100, can_hwi_handler, &hwiParams, &eb);
    } else if(ECanRegPtr == &ECanbRegs) {
        hwiHdl_can = Hwi_create(102, can_hwi_handler, &hwiParams, &eb);
    } else {
        return CO_ERROR_ILLEGAL_ARGUMENT;
    }
    if((Error_check(&eb) == TRUE) || (hwiHdl_can == NULL)) {
    ...
    if((Error_check(&eb) == TRUE) || (CANmodule->mtxHdl_can_emcy == NULL)) {
        System_printf("GateMutex_create() failed for CAN EMCY\n");
        BIOS_exit(0);
    }
    ...
    if((Error_check(&eb) == TRUE) || (CANmodule->mtxHdl_can_od == NULL)) {
        System_printf("GateMutex_create() failed for CAN OD\n");
        BIOS_exit(0);
    }
